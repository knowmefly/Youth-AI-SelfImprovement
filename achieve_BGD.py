...
def normalization(X):
    ...
    #print(X.shape)
    #print(Xmin.shape,Xmax.shape,Xmu.shape)                                         # 请补全 通过使用 np.mean 函数，计算原始数据均值，并赋值给 Xmu                 
...
def plotDecisionBoundary(trainX, trainY, w, iter_num = 0):
    ...
    #print(x_min-delta_x/10,x_max+delta_x/10)
    ...
    y = (-w[0]-w[1]*x)/w[2]
    #print(x,y,w)                                             # 通过公式计算得到直线的纵坐标： y = (-w[0]-w[1]*x)/w[2]
...
def loss(X, Y, w):  
    #print(X.shape, Y.shape, w.shape)                                # 定义一个 损失函数 loss 函数 （loss function），输入参数是 X, Y, w， 返回的是 损失函数的值
    ...
    for i in range(m):                              # 通过 for 循环结构，遍历数据集 Y_ 所有的行，其中，i 可取的数依次为：0，1 ，2，....，m-1， 数据集 Y_ 总共有 m 行    
        ...
        sum_err -=  Y[i]*np.log(Y_[i])+(1-Y_[i])*np.log(1-Y_[i])
...
def BGD(X, y, iter_num, alpha):
    ...
    for i in range(iter_num):                       # 通过 for 循环结构，开始进行迭代，其中，i 可取的数依次为：0，1 ，2，....，iter_num-1， 迭代次数总共有 iter_num 次
        ...
        w = w - (1.0/m)*alpha*trainMat.T*error                          # 更新权重 w , BGD 批量梯度下降法 的核心， w = w - (1.0/m)*alpha*trainMat.T*error
    return w                                        # 返回 w
