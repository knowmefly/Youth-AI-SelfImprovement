...
def initialize_parameters(): # 定义一个 initialize_parameters 函数
    W = tf.Variable(tf.truncated_normal([784, 1]))  #调用tf.Variable函数，设置模型参数W，W的维度为[784,1]，且初始化为0
    b = tf.Variable(tf.truncated_normal([1, 1]))  #调用tf.Variable函数，设置模型参数b，b的维度为[1  ,1],且初始化为0
...
def forward_propagation(X,parameters):  # 定义一个 forward_propagation 函数
    ...
    #print(X,W,b)
    Z1 = tf.matmul(X, W) + b  #调用tensorflow函数，实现Z1=X*W+b
    A1=tf.nn.sigmoid(Z1)  #调用tf.nn.sigmoid，实现A1 = sigmoid(Z1)
    A1 = tf.clip_by_value(A1, 0.001, 1.0)  #调用clip_by_value，将A1进行裁剪，使其在[0.001，1.0]之间，是为了避免出现接近于0的极小值，输入np.log()中出现nan的情况
    #print(A1.shape)
    return A1 # 返回 A1
...
def compute_cost(y_,y,W):  # 定义一个 compute_cost 函数
    ...
    #print(y_.shape,y,W)
    #cross_entropy = 0.1*(1.0-y_)*tf.log(1.0-y)
    #sum_err = Y[i]*np.log(Y_[i])+(1-Y[i])*np.log(1-Y_[i])
    #E1 = -y_*np.log(y)-(1-y_)*np.log(1-y)
    #y_*tf.log(y)
    #y = y*W
    cross_entropy = (1.0-y_)*tf.log(1.0-y)
    #print(cross_entropy.shape)
    #以下添加了正则，也可以尝试去掉
    #print(y_,y,W)
    cross_entropy = -(1.0/batchSize)*tf.reduce_mean(cross_entropy) #调用tf.reduce_sum函数，实现交叉熵函数
...
def model(mnist,Num): # 定义一个  model 函数
    ...
    global_step = tf.Variable(0.0)  # 调用  tf.Variable 函数， 初始化 global_step 变量
...
def test_model(): # 定义 test_model 函数
    ...
    for i in range(classNum): # for 循环语句， 遍历所有 classNum的类别， 
        ...
        a = modelNames[i]
        pf.write(str(a)) # 写入 modelNames[i]
        pf.write("\n") # 写入 换行字符
    pf.close() # 关闭文件
