...
y_ = tf.placeholder(tf.float32, shape=[None, 10])  #申请占位符 输入label N*10的矩阵[None, 10]
...
A1 = tf.layers.dense(inputs=x, units=16,activation=tf.nn.relu)  #{0}为待补充, 添加全连接层，神经元个数为16个，激活函数为sigmoid、tanh或relu
A2 = tf.layers.dense(inputs=A1,units=16,activation=tf.nn.relu)  #{0}为待补充，添加全连接层，神经元个数为16个，激活函数为sigmoid、tanh或relu
y  = tf.layers.dense(inputs=A2,units=10, activation=tf.nn.softmax)  #{0}为待补充，添加全连接层，设置激活函数为sigmoid或softmax，由于输出类别是10，所以输出层神经元个数为10
...
train_step = tf.train.GradientDescentOptimizer(0.01).minimize(cross_entropy) #设置随机梯度下降的学习率为0.01，最小化目标函数为cross_entropy

# 创建Session，用于启动tensor图
sess = tf.InteractiveSession()

# 调用global_variables_initializer函数，将前面定义的Variable变量按照设置的初始化方式，进行初始化
sess.run(tf.global_variables_initializer())  #执行tf.global_variables_initializer()，初始化模型参数
