...
def calculate_loss(model, X, y):
    ...
    a1= X  # 将特征矩阵X赋值给a1
    z2 = np.dot(W1,a1)  # 参数矩阵W1与a1做矩阵乘法，得到z2矩阵
    a2 = sigmoid(z2)  # 对z2矩阵进行sigmoid激活函数处理得到激活后的矩阵a2，即第一层隐藏层数值
    a2 = add_row(a2)  # 为矩阵a2增加一列值为1的偏置
    z3 = np.dot(W2,a2)  # 参数矩阵W2与a2做矩阵乘法，得到z3矩阵
    a3 = sigmoid(z3)  # 对z3矩阵进行sigmoid激活函数处理得到激活后的矩阵a3，即第二层隐藏层数值
    a3 = add_row(a3)  # 为矩阵a3增加一列值为1的偏置
    z4 = np.dot(W3,a3)  # 参数矩阵W3与a3做矩阵乘法，得到z4矩阵
    a4 = sigmoid(z4)  # 对z4矩阵进行sigmoid激活函数处理得到激活后的矩阵a4，即输出值
...
def build_model(X, y, hidden1_dim,hidden2_dim, iterNum=2000, print_loss=False):
    ...
    for t in range(0, iterNum):  # 从0循环至iterNum
        ...
        a2 = sigmoid(z2)  # 对矩阵z2进行做sigmoid激活
        #print(a2)
        a2 = np.vstack((np.ones((1, a2.shape[1])), a2))  # 为第一层隐藏层的矩阵a2增加一行值为1的偏置
        #print(W2.shape)
        z3 = np.dot(W2,a2)  # 参数W2与a2做矩阵乘法
        a3 = sigmoid(z3)  # 对矩阵z3做sigmoid激活
        a3 =  np.vstack((np.ones((1, a3.shape[1])), a3)) # 为第二层隐藏层的矩阵a3增加一列值为1的偏置
        z4 = np.dot(W3,a3)  # 参数W3与a3做矩阵乘法
        a4 = sigmoid(z4)  # 对矩阵z4做sigmoid激活
